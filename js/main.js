...
// ── מודאל רכישת כרטיס (checkout-modal) ────────────────────────────────
// נפתח על אותו עמוד (לא ניווט לדף) כדי שסגירה תחזיר בדיוק לאותו מסך.
// "השארת פרטים" סוגר אותו ופותח את מודאל ההצטרפות (join). בלי JS -
// כפתורי הרכישה נופלים ל-checkout.html (data-checkout על <a href>).
safe('checkout-modal', () => {
  const dialog = document.getElementById('checkoutDialog');
  const triggers = document.querySelectorAll('[data-checkout]');
  if (!dialog || !triggers.length || typeof dialog.showModal !== 'function') return;

  const open = (e) => {
    if (e) e.preventDefault();
    // אם פתוח מודאל אחר (למשל "פרטים נוספים") - לסגור לפני הפתיחה
  ...
  triggers.forEach((b) => b.addEventListener('click', open));
  const closeBtn = dialog.querySelector('[data-checkout-close]');
  if (closeBtn) closeBtn.addEventListener('click', close);
  dialog.addEventListener('click', (e) => { if (e.target === dialog) close(); });

  // "השארת פרטים" בתוך מודאל התשלום - סוגר אותו ופותח את טופס ההצטרפות
  const leave = dialog.querySelector('[data-leave-details]');
  if (leave) leave.addEventListener('click', () => {
    close();
    const joinDlg = document.getElementById('joinDialog');
    if (joinDlg && typeof joinDlg.showModal === 'function') {
      joinDlg.showModal();
      const first = joinDlg.querySelector('input[name="name"]');
      if (first) setTimeout(() => first.focus(), 60);
      track('join_open');
    }
  });
