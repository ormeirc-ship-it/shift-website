...
if (!wLive.error) {
  for (const [wk, lv] of wLive) {
    ...
    // תיאור שבוע 1: איתי הוסיף "והמוח" ישירות באתר (f7a1225, ‏10.8) -
    // אחרי עדכון-המסד האחרון (6.8). ההבדל היחיד אומת מילה-במילה ‏14.9;
    // עריכת-אתר מכוונת מאותה משפחה כמו שבוע 3, מוחרגת באותו כלל.
    if (wk !== 3 && wk !== 1 && lv.desc !== st.desc) {
