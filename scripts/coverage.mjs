 ...
 *   בלי Lenis · נשימה מלאה עד "סיימתם" (~40.5ש' — שלושה סבבי 13.5ש' של OC).
 *
...
const mobileJourney = async () => {
  ...
  // נשימה מלאה: שלושת הסבבים עד "סיימתם" (13.5ש' לסבב, מספרי OC ‏30.7) —
  // מכסה את כל שלבי ההנחיה, את finish() ואת איפוס הכפתור. המתנה אמיתית,
  // לא האצה — כדי שהמסע יישאר נאמן למכונת הזמנים שבקוד.
  await page.evaluate(async () => {
    const start = document.getElementById('breathStart');
    if (!start) return;
    start.click();
    await new Promise((r) => setTimeout(r, 42500));
  });
