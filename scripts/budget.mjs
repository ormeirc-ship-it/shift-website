...
const BUDGET = {
  'מובייל 390': {
    // ‏14.9 (המיזוג): הפיצ'רים של איתי (8-9/2026) - טלמטריה (SDK+מתאם),
    // ‏i18n (בורר-שפה), מודאל-תשלום/לידים ותוכן - באוויר מאוגוסט כחלק
    // מהמוצר. יישור מכוון למצב בפועל: קריטי נמדד 199KB + ~3% ·
    // ‏JS נמדד 258KB + ~3% · ‏CSS נמדד 116KB + ~3%.
    critical: 205 * 1024,
    // ‏29.7 (T14): הרצף הוחלף למקור ה-4K שבחר OC — ‏m ‏1.3→2.26MB (webp).
    // עדכון מכוון: נמדד 2172KB אחרי ההחלפה + ~12%. הקריטי לא זז.
    total: 2.4 * 1024 * 1024,
    js: 266 * 1024,
    css: 120 * 1024,          // כולל ה-CSS וקובצי הגופן של Google
    images: 120 * 1024,       // בלי רצף המוח
  },
  'דסקטופ 1440': {
    critical: 212 * 1024,     // ‏14.9: נמדד 206KB + ~3% (אותו יישור)
    // ‏29.7 (T14): ‏d ‏1.9→3.25MB (webp) — עדכון מכוון באותו אישור
    total: 4.4 * 1024 * 1024,
    // ‏14.9: אותו יישור מכוון כמו במובייל
    js: 266 * 1024,
    css: 120 * 1024,
    images: 120 * 1024,
  },
