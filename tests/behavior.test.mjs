...
test('מכונת הנשימה: שלושה סבבים, נעילת לחיצה-כפולה, וסיום שמוביל הלאה', () => {
  ...
  // (מחזור 13.5ש'), בשני המסלולים. שינוי שלהם הוא החלטת OC, לא תוצר לוואי.
  assert.match(main, /duration: 5\.0.*[\s\S]*?duration: 1\.5[\s\S]*?duration: 7\.0/,
    'משכי המסלול המונפש סטו ממספרי OC (5.0/1.5/7.0)');
  assert.match(main, /setTimeout\(next, 13500\)/,
    'מחזור המסלול השקט סטה מ-13.5 שניות (5.0+1.5+7.0)');
  assert.match(main, /, 5000\)[\s\S]*?, 6500\)/,
    'נקודות החלפת ההנחיה במסלול השקט סטו מ-5.0/6.5 שניות');
