...
test('מכונת הנשימה: שלושה סבבים, נעילת לחיצה-כפולה, וסיום שמוביל הלאה', () => {
  ...
  // (מחזור 13ש'), בשני המסלולים. שינוי שלהם הוא החלטת OC, לא תוצר לוואי.
  assert.match(main, /duration: 4\.0.*[\s\S]*?duration: 1\.5[\s\S]*?duration: 7\.5/,
    'משכי המסלול המונפש סטו ממספרי OC (4.0/1.5/7.5)');
  assert.match(main, /setTimeout\(next, 13000\)/,
    'מחזור המסלול השקט סטה מ-13 שניות (4.0+1.5+7.5)');
  assert.match(main, /, 4000\)[\s\S]*?, 5500\)/,
    'נקודות החלפת ההנחיה במסלול השקט סטו מ-4.0/5.5 שניות');
