...
test('B1: ערימת הקלפים מותנית במדידה, והקלפים אטומים', () => {
  ...
  // ‏2ב (2.8): המשטח עבר ממשתנה מונפש לשתי מדרגות אטומות - הקלף חייב
  // רקע hex מלא (לא rgba, לא var) בשני המצבים.
  assert.match(css, /\.week-card\s*\{[^}]*background:\s*#[0-9A-Fa-f]{6}/,
    'רקע הקלפים חזר להיות שקוף/משתנה - טקסט של קלף מכוסה ייראה דרכו, והניגודיות המדורגת של 2ב נשברת');
  assert.match(css, /\.week-card\.lit\s*\{[^}]*background:\s*#[0-9A-Fa-f]{6}/,
    'מדרגת הנייר של הקלף (2ב) נעלמה - חוזר האזור המת של שבוע 2');
